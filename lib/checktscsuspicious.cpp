            ...
            while (tok && tok != mytok->link())
            {
                if (tok->str() == ",") {
                    ...
                    if (argcnt == 2)
                    {
                        ...
                        if (arg1tok) {
                            const Variable * var =
                                _tokenizer->getSymbolDatabase()
                                ->getVariableFromVarId(arg1tok->varId());
                            if (var) {
                            const Token* toktmp = var->nameToken();
                                if (toktmp && toktmp->next()->str() == "[")
                                {
                                    toktmp = toktmp->next();
                                    if (toktmp && toktmp->next() && toktmp->next()->tokType()
                                        == Token::eNumber)
                                    {
...
void CheckTSCSuspicious::formatbufoverrunError(const Token *tok)
{
    ...
    ////    ErrorType::BufferOverrun, "formatbufoverrun", "Array index " + tok->str()
    ////    + " may be out of range.",
    ////    ErrorLogger::GenWebIdentity(tok->str()));
    reportError(tok, Severity::error,
        "formatbufoverrun",
        "Array index " + tok->str()
        + " may be out of range.", CWE0, false);
}
...
void CheckTSCSuspicious::checkNestedLoop()
{
    ...
    for (scope = symbolDatabase->scopeList.begin(); scope != symbolDatabase->scopeList.end();
        ++scope)
    {
        Token *tok;
        bool flag = false;
        ...
        else if (scope->type == Scope::eWhile)
        {
            ...
            {
                if (Token::Match(tok, "%name% <|>|<=|>=|!= %any%") || Token::Match(tok,
                    "%name% ++|-- <|>|<=|>=|!= %any%"))
                {
                    LoopVars.push_back(tok->str());
                }
        ...
        if (flag)
        {
            while (tok != NULL && tok != scope->classEnd)
            {
                if (tok->str() == "while")
                {
                    Token *temptok1 = tok->next()->link();
                    //find ")",the loop control end
                    if (temptok1 == NULL) {
                        continue;
                    }
                    Token *temptok2 = temptok1->next()->link();
                    //find "}",the for or while end
                    while (tok != temptok1)//ignore TSC
                    {
                        if (Token::Match(tok, "%name% <|>|<=|>=|!= %any%")
                            || Token::Match(tok, "%name% ++|-- <|>|<=|>=|!= %any%"))
                        {
                            ...
                            if (!tempstr.empty())
                            {
                                for (Token *temptok = temptok1->link(); temptok
                                    && temptok != temptok2; temptok = temptok->next())
                                {
                                    if ((temptok->str() == tempstr)
                                        && (Token::Match(temptok, "%any% = %any%")
                                        || Token::Match(temptok, "%any% ++|+=|--|-=")
                                        || Token::Match(temptok, "%any% = %any% +|-")
                                        || Token::Match(temptok, "--|++ %any%")))
                                    {
                                        NestedLoopError(tok);
                ...
                else if (tok && tok->str() == "for")
                {
                    ...
                    while (tok && tok != temptok1)//ignore TSC
                    {
                        ...
                        if (Token::Match(tok, "%any% <|>|<=|>=|!= %any%") && seminum == 1)
                        {
                            ...
                            if (!tempstr.empty())
                            {
                                for (Token *temptok = temptok1->link();
                                    temptok != temptok2; temptok = temptok->next())
                                {
                                    if ((temptok->str() == tempstr)
                                        && (Token::Match(temptok, "%any% = %any%")
...
void CheckTSCSuspicious::NestedLoopError(const Token *tok)
{
   ...
   ////     ErrorType::Suspicious, "NestedLoop",
    ////    "Using  same loop index variable [" + tok->str() + "] \
    ////    in a nested loop can cause loop confusion or an infinite loop.",
    ////    ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckTSCSuspicious::suspiciousArrayIndex()
{
    ...
    for (std::list<Scope>::const_iterator i = symbolDatabase->scopeList.begin();
        i != symbolDatabase->scopeList.end(); ++i) {
        if (i->type == Scope::eFor)
        {
            ...
            {
                if (wrongindex_varid && 
                    (Token::Match(tok, "%varid% --", wrongindex_varid)
                    || Token::Match(tok, "%varid% =", wrongindex_varid)))
                {
                    break;
                }
                if (Token::Match(tok, "[ %var% ]"))
                {
                    ...
                    if (tok->varId() == wrongindex_varid && wrongindex_varid != 0)
                    {
                        ...
                        if (tokBack->str() == "}" && tokBack->link())
                        {
                            ...
                            {
                                if (pTok->linenr() != tok->linenr() 
                                    && Token::Match(pTok, "%var% ++|--|=")
                                    && pTok->varId() == wrongindex_varid)
                                {
                                    break;
                                }
                                if (pTok->linenr() != tok->linenr()
                                    && Token::Match(pTok, "[ %var% ]")
                                    && pTok->tokAt(1)->varId() == wrongindex_varid)
                                {
                                    flag = false;
                                    break;
                                }
...
{
   //// reportError(tok, Severity::warning,
   ////     ErrorType::Suspicious, "suspiciousArrayIndex",
   ////     "Array index " + tok->str() + " may be out of range.",
   ////     ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckTSCSuspicious::unsafeFunctionUsage()
{
    ...
    _funcLists.insert(valType("localtime", "localtime_r"));
   /* _funcLists.insert(valType("gets", "fgets"));
    ...
    for (std::map<std::string,std::string>::const_iterator func_it = _funcLists.begin();
        func_it != _funcLists.end(); ++func_it)
    {
        for (const Token *tok = _tokenizer->tokens(); tok; tok = tok->next())
        {
...
void CheckTSCSuspicious::unsafeFunctionUsageError(const Token *tok,
    const std::string& unsafefuncname, const std::string& safefuncname)
{
    //reportError(tok, Severity::information, ErrorType::UnsafeFunc, "unsafeFunctionUsage",
    //    "High Risk to  use " + unsafefuncname + "().\
    //    It must be replaced by " + safefuncname, ErrorLogger::GenWebIdentity(unsafefuncname));
    reportError(tok, Severity::warning,
        "unsafeFunctionUsage",
        "High Risk to use " + unsafefuncname + 
        " , It must be replaced by "
            + safefuncname, CWE0, false);
}
...
void CheckTSCSuspicious::checkUnconditionalBreakinLoop()
{
    ...
    for (std::list<Scope>::const_iterator scope = symbolDatabase->scopeList.begin();
        scope != symbolDatabase->scopeList.end(); ++scope)
    {
        const Token* const toke = scope->classDef;
        if (toke && (scope->type == Scope::eFor
            || scope->type == Scope::eWhile || scope->type == Scope::eDo))
        {
            ...
            bool flag = true;
            for (Token *tok = (Token*)scope->classStart; tok != scope->classEnd && tok;
                tok = tok->next())
            {
                ...
                {
                    if (tok->scope()->type == Scope::eFor || tok->scope()->type == Scope::eWhile
                        || tok->scope()->type == Scope::eDo)
                    {
...
void CheckTSCSuspicious::UnconditionalBreakinLoopError(const Token *tok)
{
   ...
   //// reportError(tok, Severity::warning, ErrorType::Suspicious,
    ////    "unConditionalBreakinLoop", "An unconditional 'break/return/goto' \
    ////    within a loop.It may be a mistake.",
    ////    ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckTSCSuspicious::SuspiciousPriorityError(const Token *tok)
{
   ...
   ////     "SuspiciousPriority", 
   ////     "Priority of the '&&' operation is higher than that of the '||' \
   ////      operation.It's possible that parentheses should be used in the expression.",
    ...
    for (const Token *tok = _tokenizer->tokens(); tok; tok = tok->next())
    {
        ...
        if (tok && Token::Match(tok, "<<"))
        {
            ...
            if (tok->tokAt(-2) && Token::Match(tok->tokAt(-2), ";|{|}|,"))
            {
                ...
                const Variable * var =
                    _tokenizer->getSymbolDatabase()->getVariableFromVarId(tok1->varId());
                ...
                if (var)
                {
                    ...
                    if (myToken && myToken->tokAt(-1) 
                        && Token::Match(myToken->tokAt(-1),
                        "ofstream|std::ofstream|ifstream \
                        |std::ifstream|fstream|std::fstream \
...
void CheckTSCSuspicious::SuspiciousPriorityError2(const Token *tok)
{
    ////tsc
    /*reportError(tok, Severity::style, ErrorType::Suspicious,
        "SuspiciousPriority","The priority of the '+' operation 
        is higher than that of the '<<' operation. It's possible 
        that parentheses should be used in the expression.",\
        0U, "+,<<");*/
}
...
void CheckTSCSuspicious::checksuspiciousStrErrorInLoop()
{
    ...
    for (std::list<Scope>::const_iterator i = symbolDatabase->scopeList.begin();
        ...
        if (i->type == Scope::eWhile)
        {
            ...
            for (Token *tok = (Token*)i->classStart; tok
                && tok != i->classEnd; tok = tok->next())
            {
                if (tok && tok->varId() > 0)
                {
                    ...
                    if (myToken && myToken->varId() && myToken->varId() > 0 && myToken->tokAt(-1) 
                        && Token::Match(myToken->tokAt(-1),
                        "ofstream|std::ofstream|ifstream|std::ifstream|fstream \
                        |std::fstream|ostringstream|std::ostringstream|istringstream \
                        |std::istringstream|stringstream|std::stringstream"))
                    {
                        // not in protected
                        if (protectedvars.find(myToken->varId()) == protectedvars.end())
                        {
                            vars.insert(myToken);
                        }
            ...
            for (tempitor = vars.begin(); tempitor != vars.end(); tempitor++)
            {
                if (*tempitor)
                {
                    ...
                    if (tCompre&&tCompre->varId() > 0 
                        && (protectedvars.find(tCompre->varId()) == protectedvars.end()))
                    {
...
void CheckTSCSuspicious::checksuspiciousfor()
{
        ...
        i != symbolDatabase->scopeList.end(); ++i)
    {
        if (i->type == Scope::eFor)
        {
            ...
            int denum = 0;
            int ischeck = 1;
            for (Token *tok = (Token*)i->classDef; tok != i->classStart && tok;
                tok = tok->next())
            {
                if (tok->str() == ";")
                {
                ...
                if (Token::Match(tok, "%any% = %any%") && denum == 1)
                {
                    ...
                    for (infoitr1 = myinfo.begin();infoitr1 != myinfo.end(); infoitr1++)
                    {
                        ...
                        {
                            //(p=Macro(p,ss))!=null
                            if (tok->tokAt(3) && (tok->strAt(3) == "(")
                                && tok->tokAt(2)->isExpandedMacro())
                            {
                                continue;
                            }
                            ...
                            else if ((tok->tokAt(1)->astOperand2()
                                && tok->tokAt(1)->astOperand2()->tokType()
                                == Token::eComparisonOp)
                                || (tok->tokAt(1)->astParent()
                                && (tok->tokAt(1)->astParent()->tokType()
                                == Token::eComparisonOp || tok->tokAt(1)->astParent()->str()
                                == ",")))
                            {
                                continue;
                            }
                ...
                if (ischeck == 1)
                {
                    ...
                    if (Token::Match(tok, "%any% = %any% +") && denum == 2)
                    {
                        ...
                        for (itr = myinfo.begin(); itr != myinfo.end(); itr++)
                        {
                            if (tok->varId() == itr->forid && tok->tokAt(2)->varId()
                                == itr->forid)
                            {
                                itr->twostrs = "+";
                            }
            ...
            {
                if ((infoitr->onestrs == "<" || infoitr->onestrs == "<=")
                    && infoitr->twostrs == "-")
                {
...
{
    ////reportError(tok, Severity::warning,
    ////    ErrorType::Suspicious, "suspiciousfor", "suspicious this 'for' has error.",
    ////    ErrorLogger::GenWebIdentity(tok->str()));
}
...
{
   //// reportError(tok, Severity::warning,
  ////      ErrorType::Suspicious, "suspiciouswhilestrerror",
  ////      "suspicious string error in loop;Do you forget to set string empty?",
  ////      ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckTSCSuspicious::checkwrongvarinfor()
{
        ...
        i != symbolDatabase->scopeList.end(); ++i) 
    {
        if (i->type == Scope::eFor)
        {
            ...
            for (Token *tok = (Token*)i->classDef; tok && tok != i->classStart;
                tok = tok->next())
            {
                if (tok->str() == ";")
                {
                ...
                if (Token::Match(tok, "%any% <|>|<=|>=|!=|== %any%") && denum == 1)
                {
                    ...
                    while (tok && tok->str() != ";" && tok->str() != "&&" && tok->str() != "||")
                    {
                        ...
                        if (tok->tokType() == Token::eVariable && tok->varId() == tmpvarid
                            && (vars.find(tok->varId()) != vars.end()))
                        {
                ...
                //for (n = info->dlpi_phnum; --n >= 0; phdr++) the second term varies
                if (Token::Match(tok, "%var% ++|+=|--|-=|=")
                    && denum == 1)
                {
                ...
                //for (n = info->dlpi_phnum; --n >= 0; phdr++) the second term varies
                if (Token::Match(tok, "++|-- %var%") && denum == 1)
                {
                    if (tok->next()->varId() != 0 && vars.find(tok->next()->varId())
                        != vars.end())
                    {
                        ischeck = true;
                        break;
                ...
                if (denum == 2 && tok && tok->str() == ";"
                    && tok->tokAt(2) && tok->tokAt(2) == i->classStart)
                {
                    ischeck = true;
                    break;
                }
                if (denum == 2)
                {
                    ...
                    for (Token* tempTok = tok; tempTok && tempTok != i->classEnd;
                        tempTok = tempTok->next())
                    {
                        ...
                        tempTok = tempTok->next())
                    {
                        ...
                        if (Token::Match(tempTok, "++|-- %var%"))
                        {
                            if (tempTok->tokAt(1)->varId() != 0
                                && vars.find(tempTok->tokAt(1)->varId()) != vars.end())
                            {
                                ischeck = true;
                            }
                            //solve varid=0 because of syntax error like register i;
                ...
                if (denum == 2)
                {
                    for (Token* tempTok = tok; tempTok
                        && tempTok != i->classStart; tempTok = tempTok->next())
                    {
...
void CheckTSCSuspicious::wrongvarinforError(const Token *tok)
{
  ...
  ////      "Invalid use of index["+tok->str()+"] in 'for' statement leads to dead loop.",
   ////     ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckTSCSuspicious::wrongvarinforError2(const Token *tok)
{
 ////   reportError(tok, Severity::warning,
    ...
    ////    "wrongvarinfor", "Invalid use of index[" + tok->str() + "] in 'for' \
   ////     statement leads to dead loop. The condition in 'for statement' \
   ////     like 'i<i+2' or 'i<a[i].length' may be an error. ", 
   ////     ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckTSCSuspicious::checkFuncReturn()
{
        ...
        if (nametok->previous() != NULL) {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (dedeTok && dedeTok->previous() != NULL
                        && dedeTok->previous()->str() == ">")
                    {
                        dedeTok = dedeTok->previous()->link();
                        if (dedeTok != NULL && dedeTok->tokAt(-2) != NULL
                            && dedeTok->tokAt(-2)->str() != "void")
                        {
                            hasreturn = true;
                        }
                    }
                    ...
                    {
                        if (nametok->tokAt(-5) != NULL && nametok->tokAt(-5)->str() != "::"
                            && nametok->tokAt(-5)->str() != "void")
                        {
                            hasreturn = true;
                        }
                    }
                    ...
                    {
                        if (nametok->tokAt(-3) != NULL && nametok->tokAt(-3)->str()
                            != "void")
                        {
                            hasreturn = true;
                        }
    ...
    for (std::vector<const Scope *>::const_iterator i = scope.begin(); i != scope.end(); ++i)
    {
        for (const Token* tok = (*i)->classStart; tok != (*i)->classEnd; tok = tok->next())
        {
            ...
            {
                if (tok->next()->link()->next() != NULL
                    && Token::Match(tok->next()->link()->next(), "."))
                {
                    continue;
                }
                //if (!Token::Match(tok,"if|while|switch|=|for|return"))
                if (tok->tokType() == Token::eFunction && !Token::Match(tok,
                    "return|printf|assert"))
                {
                    if (tok->next()->link() && Token::simpleMatch(tok->next()->link()->next(), "{")
                    ...
                    Token* tmp = NULL;
                    if (tok->next()->link() != NULL)
                    {
                        tmp = tok->next()->link()->next();
                    }
                    ...
                    for (const Token* temp = tok; temp != NULL && !Token::Match(temp, ";|}|{");
                        temp = temp->previous())
                    {
                        if (Token::Match(temp,
                    ...
                    if (!isfind)
                    {
                        ...
                        {
                            for (Token* ftemp = fstart; ftemp != fend && ftemp;
                                ftemp = ftemp->next())
                            {
                                ...
                                if (ftemp->str() == ",")
                                {
                                    ...
                                    if (mytok != NULL) {
                                    const Variable *var =
                                        symbolDatabase->getVariableFromVarId(mytok->varId());
                                    if (var != NULL)
                                    {
                        ...
                        for (itrfunlist = funlist.begin(); itrfunlist != funlist.end(); 
                            itrfunlist++) {
                            if (itrfunlist->argNum == nowfunc.argNum && itrfunlist->funcname
                                == nowfunc.funcname)
                            {
                                std::vector<string>::iterator itrtype;
                                int i2 = 0;
                                bool isok = true;
                                for (itrtype = itrfunlist->argType.begin(); itrtype
                                    != itrfunlist->argType.end(); itrtype++)
                                {
                                    if (*itrtype != nowfunc.argType[i2] && nowfunc.argType[i2]
                                        != "UNKOWN")
                                    {
                                        isok = false;
                                        break;
...
{
   //// reportError(tok, Severity::style,
   ////     ErrorType::Suspicious, "FuncReturn", "The return value of function [" + tok->str() + "] \
   ////     is not used.", ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckTSCSuspicious::checkboolFuncReturn()
{
    bool hasreturn;
    const SymbolDatabase *symbolDatabase = _tokenizer->getSymbolDatabase();
    for (std::vector<const Scope *>::const_iterator i = symbolDatabase->functionScopes.begin();
        i != symbolDatabase->functionScopes.end(); ++i)
    {
        hasreturn = false;
        const Token* nametok = (*i)->classDef;
        ...
        if (hasreturn)
        {
            for (const Token* tok = classsbegin; tok && tok != (*i)->classEnd;
                tok = tok->next())
            {
                // inner class
...
void CheckTSCSuspicious::boolFuncReturnError(const Token *tok)
{
    if (tok) {
      ////  reportError(tok, Severity::style,
     ////       ErrorType::Suspicious, "BoolFuncReturn", "Returning type of this function \
     ////       is expected to be bool,but non-bool variable [" + tok->str() + "] is returned.",
     ////       ErrorLogger::GenWebIdentity(tok->str()));
    }
}
...
void CheckTSCSuspicious::checkIfCondition()
{
        ...
        i != symbolDatabase->scopeList.end(); ++i)
    {
        ...
        {
            for (Token *tok3 = (Token*)i->classDef->tokAt(2); tok3 != i->classStart
                && tok3; tok3 = tok3->next())
            {
                if (tok3->str() == "(")
                {
                ...
                if (Token::Match(tok3, "%any% = %any%") && tok3->tokAt(3)
                    && tok3->tokAt(3)->str() != ",")
                {
                    Token *valueTok = tok3->tokAt(2);
                    if (valueTok && valueTok->isNumber() && valueTok->next()->tokType()
                        != Token::eArithmeticalOp)
                    {
                        isNum = 1;
                    }
                    ...
                    //bool bDeng = false;
                    while (!Token::Match(downtok1, "||") && !Token::Match(downtok1, "&&")
                        && downtok1 != i->classStart)
                    {
                    ...
                    while (!Token::Match(uptok2, "||") && !Token::Match(uptok2, "&&")
                        && uptok2 != i->classDef && bLogicalOp)
                    {
...
void CheckTSCSuspicious::IfConditionError(const Token *tok)
{
   ...
   //// reportError(tok, Severity::warning,
   ////     ErrorType::Suspicious, "IfCondition", "this Condition Contain '=' and \
...
void CheckTSCSuspicious::checkRenameLocalVariable()
{
    ...
    for (std::size_t i = 0; i < functions; ++i) {
        ...
        for (const Token *tok = scope->classDef->next(); tok != scope->classEnd;
            tok = tok->next()) {
            int varid = tok->varId();
...
{
    ////reportError(tok, Severity::warning,
    ////    ErrorType::Suspicious, "RenameLocalVariable", "Local variable ["+tok->str()+"] \
    ////    is renamed in current function.",
    ////    ErrorLogger::GenWebIdentity(tok->str()));
}
...
extern bool is_cast(const Token *tok);
...
bool CheckTSCNullPointer2::CanBeNull(const SExprLocation& sExprLoc, CExprValue& ev)
{
    ...
    if (const Variable* pVar = sExprLoc.get_variable())
    {
...
bool CheckTSCNullPointer2::IsDerefTok(const Token* tok)
{
    ...
    else if (tok->str() == "<<" || tok->str() == ">>")
    {
        ...
        {
            SExprLocation exprLoc = create_sexpr_by_expr_tok(tok->astOperand2());
            if (!exprLoc.Empty())
            {
...
void CheckTSCNullPointer2::HandleArrayDef(const Token *tok)
{
    ...
    if (pToken->isName())
    {
        SExprLocation loc = create_sexpr_by_expr_tok(tok->astOperand1());
...
void CheckTSCNullPointer2::HanldeDotOperator(const Token* tok)
{
    if (tok->astOperand1())
    {
        SExprLocation exprLoc = create_sexpr_by_expr_tok(tok->astOperand1());
        if (!exprLoc.Empty())
        {
...
bool CheckTSCNullPointer2::SpecialHandleForCond(const Token* tokFor)
{
    ...
    for (const Token* tok = start->next(); tok && tok != tokEnd; tok = tok->next())
    {
        ...
        {
            SExprLocation exprLoc = create_sexpr_by_expr_tok(tok->next());
            if (!exprLoc.Empty())
            {
                bool bHandled = false;
                if (!bHandled)
                ...
                if (pToken->next() && pToken->next()->str() == "{")
                {
                    ...
                    if (pToken->tokAt(2))
                    {
                        if (pTokenAt2->str() == "delete")
                        {
                            ...
                            if (tok2)
                            {
                                SExprLocation sLoc = create_sexpr_by_expr_tok(tok2);
                        ...
                        else if (pTokenAt2->str() == "free")
                        {
                            ...
                            if (pTokenAt2->next() && pTokenAt2->next()->str() == "(")
                            {
                                ...
                                {
                                    SExprLocation sLoc = create_sexpr_by_expr_tok(pTokenAt2->next()->astOperand2());
                                    if (sLoc.Expr == ev.GetExpr())
                                    {
                                        return true;
                                    }
...
const Token* CheckTSCNullPointer2::HandleSmartPointer(const Token *tok)
{
    ...
    if (tok->astParent()->str() == ".")
    {
        ...
        SExprLocation loc = create_sexpr_by_expr_tok(pTokenTop->astOperand1()->astOperand1());
        ...
        {
            SExprLocation loc2 = create_sexpr_by_expr_tok(tokParam);
            if (loc2.Empty())
            {
                return tok;
            }
        ...
        if (Token::Match(tok, "( void| )")) 
        {
            if (const Token* tok1 = tok->astOperand1())
            {
                if (Token::Match(tok1, "::|."))
                {
                    ...
                    {
                        SExprLocation exprLoc = create_sexpr_by_expr_tok(tok2);
                        if (!exprLoc.Empty())
                        {
...
bool CheckTSCNullPointer2::CheckIfDirectFuncNotRetNull(const SExprLocation& expr) const
{
    ...
    return CGlobalTokenizer::Instance()->check_if_match_signature(link->previous(),
        _settings->_functionNotRetNull, _settings->_equalTypeForFuncSig)
        //|| CheckIfLibFunctionReturnNull(link->previous())
        || CGlobalTokenizer::Instance()->check_func_return_null(link->previous()) == gt::CFuncData::RetNullFlag::fNotNull;
}
...
void CheckTSCNullPointer2::CheckDerefBeforeCondError(CCompoundExpr* ifCond, const Token *tok, const Token* tokKey)
{
    ...
    for (std::vector<CCompoundExpr*>::iterator I = listCE.begin(), E = listCE.end(); I != E; ++I)
    {
        ...
        const CExprValue& ev = ce->GetExprValue();
        const SExpr& expr = ev.GetExpr();
        if (!ev.get_possible() && (!ev.GetValueInfo().IsStrValue()) && (ev.GetValueInfo().GetValue() == 0))
        {
...
{
    if (const Variable* var = exprLoc.get_variable())
    {
        return !var->isArray();
    }
    else if (exprLoc.TokTop->str() == "(")
    {
        ...
        if (CGlobalTokenizer::Instance()->check_if_match_signature(tokFunc,
            _settings->_functionNotRetNull, _settings->_equalTypeForFuncSig))
        {
            return false;
        }
        ...
        {
            if (check_if_lib_function_return_null(tokFunc)
                || CGlobalTokenizer::Instance()->check_func_return_null(tokFunc) != gt::CFuncData::RetNullFlag::fNotNull)
            {
                return true;
            }
            else
...
void CheckTSCNullPointer2::nullPointerError(const SExprLocation& sExprLoc, const CExprValue& ev)
{
    ...
    if (ev.IsCondIterator())
    {
        ...
        reportError(sExprLoc.Begin, Severity::error,
            "invalidDereferenceIterator",
            ss.str().c_str(), CWE0, false);
    }
    ...
    else
    {
        ...
        ////    ErrorLogger::GenWebIdentity(sExprLoc.Expr.ExprStr));
        reportError(sExprLoc.Begin, Severity::error,
            "possibleDereferenceAfterCheck",
            "Possible null pointer dereference:" + sExprLoc.Expr.ExprStr, CWE0, false);
    }
}
...
void CheckTSCNullPointer2::derefBeforeCheckError(const SExprLocation& exprLoc, const CExprValue& ev)
{
    ...
    else
    {
        ...
        if (strstr(exprLoc.Expr.ExprStr.c_str(), "("))
        {
            ...
            reportError(ev.GetExprLoc().TokTop, Severity::error,
                "funcDereferenceBeforeCheck",
                ss.str().c_str(), CWE0, false);
        }
...
{
    ////tsc
    /*if (!_settings->isEnabledEx("warning"))
    {
        return;
    }*/
    ...
    if (m_possibleReportedErrorSet.find(sExprLoc.Expr) == m_possibleReportedErrorSet.end())
    {
        ...
        {
            ////tsc directFuncPossibleRetNull nullPointerArray 
            ...
            ////tsc nullPointerClas possibleNullDereferenced
...
void CheckTSCNullPointer2::possibleNullPointerError(const SExprLocation& exprLoc)
{
    ...
    reportError(exprLoc.Begin, Severity::error,
        "possibleNullDereferenced",
        "Possible null pointer dereference:" + exprLoc.Expr.ExprStr, CWE0, false);
}
...
void CheckTSCNullPointer2::GetForLoopIndexByLoopTok(const Token* tokLoop, std::vector<SExprLocation>& indexList)
{
    ...
    if (Token::Match(tokLoop, "++|--"))
    {
        ...
        {
            SExprLocation exprLoc = create_sexpr_by_expr_tok(tokExpr);
            if (!exprLoc.Empty())
            {
...
bool CheckTSCNullPointer2::AddEVToCompoundExpr(const Token* tok, const CValueInfo& info, CCompoundExpr* compExpr, bool bIterator)
{
    if (tok->str() == "=")
    {
        SExprLocation exprLoc1 = create_sexpr_by_expr_tok(tok->astOperand1());
        SExprLocation exprLoc2 = create_sexpr_by_expr_tok(tok->astOperand2());
    ...
    {
        SExprLocation exprLoc = create_sexpr_by_expr_tok(tok);
        if (exprLoc.Empty())
        {
            return false;
        }
...
bool CheckTSCNullPointer2::HandleRetParamRelation(const SExprLocation& exprLoc, const SExprLocation& exprLoc2, const gt::CFunction *gtFunc)
{
    ...
    const std::set<gt::CFuncData::ParamRetRelation> *pRetParam = CGlobalTokenizer::Instance()->check_ret_param_relation(gtFunc);
    if (!pRetParam)
    {
        return false;
    }
    ...
    std::vector<const Token*> args;
    get_params_by_ast(exprLoc2.TokTop, args);
    bool bHandled = false;
    ...
    for (std::set<gt::CFuncData::ParamRetRelation>::const_iterator iter = pRetParam[0].begin(), end = pRetParam[0].end(); iter != end; ++iter)
    {
        if ((size_t)iter->nIndex < args.size())
        {
            ...
            const SExprLocation paramLoc = create_sexpr_by_expr_tok(tokArg);
            if (paramLoc.Empty())
            {
                continue;
            }
...
void CheckTSCNullPointer2::HandleAssign(const Token * const tok, const SExprLocation& exprLoc, const CExprValue& curPre)
{
        ...
        )
    {
        ...
        {
            SExprLocation rightLoc = create_sexpr_by_expr_tok(tok->astOperand2());
            CExprValue rightVal;
    ...
    if (!handled)
    {
        ...
        if (tokValue->str() == "&")
        {
            ...
            {
                SExprLocation exprLoc2 = create_sexpr_by_expr_tok(tokValue->astOperand1());
                if (!exprLoc2.Empty())
                {
    ...
    if (!handled)
    {
        ...
        //if (!tokValue->isCast())
        {
            SExprLocation exprLoc2 = create_sexpr_by_expr_tok(tokValue);
            ...
            if (!exprLoc2.Empty())
            {
                ...
                else if (exprLoc2.TokTop->str() == "(")
                {
                    ...
                    const gt::CFunction *gtFunc = CGlobalTokenizer::Instance()->find_function_data(tokFunc);
                    if (HandleRetParamRelation(exprLoc, exprLoc2, gtFunc))
                    {
                        handled = true;
                    }
                    //make function impossible to return null if configured in cfg.xml
                    /*else if (CGlobalTokenizer::Instance()->check_if_match_signature(tokFunc, 
                        _settings->_functionNotRetNull,
                        _settings->_equalTypeForFuncSig) ||
                        CGlobalTokenizer::Instance()->check_func_return_null(tokFunc) == gt::CFuncData::RetNullFlag::fNotNull)
                    {
                        CExprValue ev2(exprLoc, CValueInfo(0, false));
                        ev2.SetCondInit();
                    ...
                    else
                    {
                        ...
                        if (check_if_lib_function_return_null(tokFunc)
                            || CGlobalTokenizer::Instance()->check_func_return_null(tokFunc) == gt::CFuncData::RetNullFlag::fNull)
                        {
                            // special handling
    ...
    if (!handled)
    {
        ...
        {
            if (const Variable* pVar = exprLoc.get_variable())
            {
                if (pVar->isPointer())
                {
                    CExprValue newEv(exprLoc, CValueInfo(0, false));
...
{
    std::vector<const Token*> args;
    get_params_by_ast(tokFuncTop, args);
    ...
    for (std::vector<const Token*>::iterator I = args.begin(), E = args.end(); I != E; ++I)
    {
        ...
        {
            SExprLocation exprLoc = create_sexpr_by_expr_tok(tokArg);
            if (!exprLoc.Empty())
            {
    ...
    if (tokFunc)
    {
        ...
        if (tokFunc->tokType() != Token::eVariable)
        {
            if (tokFunc->isName())
            {
                ...
                if (get_lib_not_null_param_index_by_name(derefArgIndex, tokFunc))
                {
                    //intentionally left blank
                }
                else
                {
                    bool bRet = CGlobalTokenizer::Instance()->check_deref_vars(tokFunc, derefedVar);
                    if (bRet)
                    {
        ...
        else
        {
            if (tokFunc->variable())
            {
                ...
                {
                    SExprLocation exprLoc = create_sexpr_by_expr_tok(tokFunc);
                    CExprValue newEv(exprLoc, CValueInfo(0, false));
                    newEv.SetCondInit();
                    m_localVar.AddPreCond(newEv, true);
...
bool CheckTSCNullPointer2::HandleRetParamRelation(const Token *tokFunc, const Token *tokBracket, const gt::CFunction *gtFunc)
{
    ...
    //at least one token
    SExprLocation loc2 = create_sexpr_by_expr_tok(tokBracket);
    if (loc2.Empty())
    {
        return false;
    }
    std::vector<const Token*> args;
    get_params_by_ast(loc2.TokTop, args);
    if (args.empty())
    {
        return true;
    }
...
bool CheckTSCNullPointer2::check_if_lib_function_return_null(const Token * const pToken) const
{
    return pToken && _settings->library.check_if_lib_function_return_null(CGlobalTokenizer::find_full_function_name(pToken));
    return true;
}


bool CheckTSCNullPointer2::get_lib_not_null_param_index_by_name(std::set<int> &derefIndex, const Token *tok) const
{
    return _settings->library.get_lib_not_null_param_index_by_name(derefIndex, CGlobalTokenizer::find_full_function_name(tok));
    return true;
}
...
{
    if (!sExprLoc.get_variable() || !sExprLoc.get_variable()->isLocal())
    {
        return false;
    }
...
void CheckTSCNullPointer2::OpenDumpFile()
{
    ...
    if (m_dump)
    {
        ...
        std::string logDir = Utility::create_log_directory();
        std::ostringstream oss;
        ...
        if (evAssigned.IsCertain())
        {
            ...
            if (!ifCond->HasSExpr(expr))
            {
                ...
                */
                ...
                if (!isSubField)
                {
                    if (!(evAssigned.GetExprLoc().get_variable() && evAssigned.GetExprLoc().get_variable()->typeStartToken()->str() == "bool" && !evAssigned.IsInitCertain()))
                    {
...
bool CheckTSCNullPointer2::TryGetDerefedExprLoc(const Token* tok, SExprLocation& exprLoc, const SymbolDatabase* symbolDB)
{
    ...
    if (tok->str() == "[" || tok->str() == "." || tok->str() == "*")
    {
        ...
        if (ltok)
        {
            ...
            exprLoc = create_sexpr_by_expr_tok(ltok);
        }
        //ast may fail
        else if (tok->str()==".")
        {
            //ROTATE_LOG ( $g_unicom_music_log , ## $ERROR , $" err(code:%s,msg:%s)\n" $, $string ( $returnCode@7 . $value ( ) ) . $c_str ( ) , $string ( $description@10 . $value ( ) ) . $c_str ( ) ) ;
            exprLoc = create_sexpr_by_expr_tok(tok->previous());
        }
    }
    else if (tok->str() == "<<" || tok->str() == ">>")
    {
        if (const Token* tok2 = tok->astOperand2())
        {
            exprLoc = create_sexpr_by_expr_tok(tok2);
        }
    }
...
const Token* CheckTSCNullPointer2::HandleOneToken(const Token * tok)
{
    ...
    else if (CGlobalTokenizer::Instance()->check_if_return(tok))
    {
        HandleReturnToken(tok);
    }
    // handle deref, todo, handle deref in subfunc , handle deref in sysfunc 
...
bool CheckTSCNullPointer2::IsIterator(const Token* tok)
{
    ...
    if (exprLoc.Expr.VarId)
    {
        const Variable* pVar = exprLoc.get_variable();
        if (pVar)
        {
...
void CheckTSCNullPointer2::checkNullDefectError(const SExprLocation& exprLoc)
{
    ...
    reportError(exprLoc.Begin, Severity::error,
        "checkNullDefect",
        "It's wrong way to check if [" + exprLoc.Expr.ExprStr + "] is null, this is usually caused by misusing && and ||, e.g. if(p != NULL || p->a == 42)", CWE0, false);
}
...
void CheckTSCNullPointer2::checkMissingDerefOperator()
{
        ...
        E = _tokenizer->getSymbolDatabase()->functionScopes.end(); I != E; ++I)
    {
        ...
        for (const Token* tok = s->classStart, *tokE = s->classEnd; tok && tok != tokE; tok = tok->next())
        {
            ...
            SExprLocation elVar = create_sexpr_by_expr_tok(tokVar);
            if (elVar.Empty())
            {
                continue;
            }
            ...
            if (tok2)
            {
                for (; tok2 && tok2 != tokE; tok2 = tok2->next())
                {
                    ...
                    SExprLocation el2 = create_sexpr_by_expr_tok(tok2);
                    ...
                    if (tokP2 && tokP2->str() == "==")
                    {
                        ...
                        //// tsc
                        if (tokValue && (tokValue->str() == "0" || tokValue->str() == "NULL")) {
                            bCheckNull = true;
                        }
                    }
...
void CheckTSCNullPointer2::reportMissingDerefOperatorError(const SExprLocation& elAssign, const SExprLocation& elCheckNull)
{
    ...
    reportError(elCheckNull.TokTop, Severity::error,
        "missingDerefOperator",
        ss.str().c_str(), CWE0, false);

...
const Variable* SExprLocation::get_variable() const
{
    if (Expr.VarId)
    {
        if (TokTop->varId())
...
std::string TokenSection::to_type_string(const char* sep /*= NULL*/) const
{
    if (Empty()) {
        return "";
    }
...
const Token* TokenSection::last_token() const
{
    if (Empty()) {
        return nullptr;
    }
...
bool TokenSection::is_one_token() const
{
...
SExprLocation create_sexpr_by_expr_tok(const Token* tok)
{
    ...
    else if (tok->str() == "(")
    {
        ...
        if (is_cast(tok))
        {
            return create_sexpr_by_expr_tok(tok->astOperand1());
        }
        if (tok->astOperand1())
        {
    ...
    else if (tok->str() == "*")
    {
        ...
        if (tok->astOperand1() && !tok->astOperand2())
        {
            ...
            {
                SExprLocation expr_loc = create_sexpr_by_expr_tok(tok->astOperand1());
                if (!expr_loc.Empty())
                {
                    expr_loc.Begin = tok;
                    expr_loc.TokTop = tok;
                    expr_loc.Expr.ExprStr = tok->str() + expr_loc.Expr.ExprStr;
                    expr_loc.Expr.VarId = 0;
                }
                return expr_loc;
            }
        }
    }
    //handle (&p[i])->xx;  AND c=((tlist_node*)((long)(&p.stLink)+(&p.stLink).iPrev))->a;  AND (la-2)->xx;
...
void get_params_by_ast(const Token* tok, std::vector<const Token*> &params)
{
    ...
    std::list<const Token*> param_list;
    tok = tok->astOperand2();
...
int try_get_expr_size(const Token* tok)
{
    ...
    {
        s = (int)get_standard_type_size(pVar->typeStartToken(), pVar->typeEndToken());
    }
    // array
    else if (tok->str() == "[" && tok->astOperand1())
    {
        s = try_get_expr_size(tok->astOperand1());
    }
    else if (tok->str() == "." && tok->astOperand2())
    {
        const Token* tok2 = tok->astOperand2();
        s = try_get_expr_size(tok2);
    }
    ////tsc
    ////else if (tok->str() == "(" && iscast(tok) && tok->astOperand1())
    else if (tok->str() == "(" && is_cast(tok) && tok->astOperand1())
    {
        s = try_get_expr_size(tok->astOperand1());
    }
    else
    {
        const gt::CField* fi = CGlobalTokenizer::Instance()->get_field_info_by_token(tok);
        if (fi)
        {
            s = get_standard_type_size(fi);
        }
    }

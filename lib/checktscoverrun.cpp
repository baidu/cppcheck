...
std::map<std::string, int> CheckTSCOverrun::s_returnLengthFunc =
make_container<std::map<std::string, int>>()
...
void CheckTSCOverrun::checkFuncRetLengthUsage()
{
    ...
    for (std::vector<const Scope *>::const_iterator I = funcs.begin(), E = funcs.end(); I != E; ++I)
    {
        ...
        for (const Token* tok = scope->classStart; tok && tok != scope->classEnd;
            tok = tok->next())
        {
                        ...
                        if (const Token* tokLen = tok2->astParent()->astOperand1())
                        {
                            if (tokLen && tokLen->varId())
                            {
                                ...
                                if (params.size() > (unsigned)paramIndex)
                                {
                                    ...
                                    for (const Token* tok3 = tok->next();
                                        tok3 && tok3 != scope->classEnd && index >= 0;
                                        tok3 = tok3->next())
                                    {
                                        if (tok3->varId() == varid)
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                if (tok4 && tok4->str() == "[")
                                                {
                                                    ...
                                                    if (tok5 && tok5->astString() ==
                                                        tokBuf->astString() && tok5->varId() ==
                                                        tokBuf->varId())
                                                    {
...
void CheckTSCOverrun::checkIndexPlusOneInFor()
{
    ...
    for (std::vector<const Scope *>::const_iterator I = funcs.begin(), E = funcs.end();
        I != E; ++I)
    {
        const Scope* scope = *I;
            ...
            if (tok1 && tok2)
            {
                ...
                if (tokArr)
                {
                    ...
                    if (tok2->str() == "<")
                    {
                        ...
                        {
                            if (tok22->isNumber() || (tok22->isName() && !tok22->variable()
                                && !Token::Match(tok22->next(), "(|.|[")))
                            {
...
void CheckTSCOverrun::checkIndexCheckDefect()
{
        ...
        I != E; ++I)
    {
        ...
        for (const Token* tok = scope->classStart;
            tok && tok != scope->classEnd; tok = tok->next())
        {
            if (tok->str() != "[")
            {
                continue;
            ...
            SExprLocation elArray = create_sexpr_by_expr_tok(tok1);
            SExprLocation elIndex = create_sexpr_by_expr_tok(tok2);
            ...
            const Variable* varArr = elArray.get_variable();
            ...
            {
                const gt::CField* gtF =
                    CGlobalTokenizer::Instance()->get_field_info_by_token(tok->previous());
            ...
            int layer1 = 0;
            int layer2 = 0;
            for (const Token* tok3 = elArray.Begin->previous();
                tok3 && tok3 != scope->classStart; tok3 = tok3->previous())
            {
                if (tok3->str() == "}")
                {
                    ++layer2;
                ...
                else if (CheckIndexTheSame(elIndex, tok3))
                {
                    if (Token::Match(tok3->next(), " %op% %any%"))
                    {
                        ...
                        else if ((tokCond->str() == "if" && layer1 > 0)
                            || (tokCond->str() == "for" && layer1 > 0))
                        {
                            if (tokOp->str() == "<=")
                            {
                                if (d1.IsNum())
...
bool CheckTSCOverrun::CheckIndexTheSame(SExprLocation elIndex, const Token* tok)
{
    ...
    {
        const Token* tmp = elIndex.last_token();
...
void CheckTSCOverrun::checkIndexPlusOneInForByScope(const Scope* scope)
{
    ...
    for (std::list<Scope*>::const_iterator I2 = subScopes.begin(), E2 = subScopes.end();
        I2 != E2; ++I2)
    {
    ...
    if (tokParenthesis && tokParenthesis->str() == "(" && tokParenthesis->link())
    {
        ...
        if (sc1 && sc1->str() == ";")
        {
            ...
            if (sc2 && sc2->str() == ";")
            {
                ...
                for (const Token* tok = sFor->classStart->next();
                    tok && tok != sFor->classEnd; tok = tok->next())
                {
                    ...
                    else if (Token::Match(tok, "[ %name% + 1 ]"))
                    {
                        if (tok->next()->varId())
                        {
                            ...
                            while (s && s != sFor)
                            {
                                ...
                                {
                                    const Token* tok2 = Token::findmatch(s->classDef,
                                        "%name% <", s->classStart);
                                    ...
                                    tok2 = Token::findmatch(s->classDef,
                                        "%name% + 1 <", s->classStart);
                                    if (tok2 && (tok2->str() == tok->next()->str()))
                                    {
                                        bBreak = true;
                ...
                {
                    const gt::CField* gtF = 
                        CGlobalTokenizer::Instance()->get_field_info_by_token(tokVar);
                ...
                {
                    if (Token::Match(tok, "++ %name%") && tok->next()->varId()
                        == tokIndex->varId())
                    {
                        bMatch = true;
                        break;
                ...
                {
                    if (Token::Match(tok, "%varid% < %any% ;|&&", tokIndex->varId())
                        && tok->strAt(2) == sLength)
                    {
...
{
    std::stringstream ss;
    ss << "Index [" << tok->str() << "] which is returned form [" << func << "] \
        may cause index out of bound.";
    ////reportError(tok, Severity::error, ErrorType::BufferOverrun, "funcRetLengthAsIndex",
    ////    ss.str(), ErrorLogger::GenWebIdentity(tok->str()) );
    reportError(tok, Severity::error,
        "funcRetLengthAsIndex",
        ss.str(), CWE0, false);
}
...
{
    std::stringstream ss;
    ss << "Index [" << tok->str() << tok->next()->str() << tok->strAt(2) << "] \
        may cause index out of bound.";
    ////reportError(tok, Severity::error, ErrorType::BufferOverrun, "arrayIndexOutOfBounds",
    ////ss.str(), ErrorLogger::GenWebIdentity(tok->str()));
    reportError(tok, Severity::error,
...
void CheckTSCOverrun::IndexBeforeCheckError(const Token* tok)
{
    ...
    ////reportError(tok, Severity::error, ErrorType::BufferOverrun,
    ////    "arrayIndexThenCheck", ss.str(), ErrorLogger::GenWebIdentity(tok->str()));
    reportError(tok, Severity::error,
        "arrayIndexThenCheck",
        ss.str(), CWE0, false);
}
...
{
    std::stringstream ss;
    ss << "[" << tokStr->str() << "] is greater or equal than size of \
    [" << "" << "] which the buffer may be full filled then no space for the terminating null character.";
    ////[" << tokVar->astString2()<< "] which the buffer may be full filled then no space for the terminating null character.";
    ...
    reportError(tokFunc, Severity::error,
...
void CheckTSCOverrun::StrncatError(const Token* tokVar)
{
    ...
    ss << "Calling strncat in for-loop may cause [" << "" << "] overrun";
    ...
    reportError(tokVar, Severity::error,
        "bufferAccessOutOfBounds",
        ss.str(), CWE0, false);
}

void CheckTSCOverrun::IndexCheckDefectError(const SExprLocation& elIndex,
    const SExprLocation& elArray, gt::CField::Dimension d, const Token* tokCond)
{
    if (m_reportedError)
    {
    ...
    std::stringstream ss;
    ss << "Array " << elArray.Expr.ExprStr << "[" << d.Output() << "] \
        is accessed at index [" << elIndex.to_type_string() << "] may out of bound, \
        as the index range check defect at line " << tokCond->linenr() << " may be wrong, \
        mind the boundary value.";
    ////reportError(elIndex.TokTop, Severity::error, ErrorType::BufferOverrun,
    ////    "arrayIndexCheckDefect", ss.str(), ErrorLogger::GenWebIdentity(elIndex.to_type_string()));
    reportError(elIndex.TokTop, Severity::error,
        "arrayIndexCheckDefect",
        ss.str(), CWE0, false);
}
...
gt::CField::Dimension CheckTSCOverrun::GetDimensionInfo(const Token* tok)
{
    ...
    {
        const gt::CField* gtF = CGlobalTokenizer::Instance()->get_field_info_by_token(tok);
...
void CheckTSCOverrun::checkStrncat()
{
        ...
        I != E; ++I)
    {
        ...
        std::set<SExpr> checkedVar;
        for (const Token* tok = scope->classStart; tok && tok != scope->classEnd; tok
            = tok->next())
        {
            if (Token::Match(tok, "strncat ("))
            {
                std::vector<const Token*> args;
                get_params_by_ast(tok->next(), args);
                if (args.size() == 3)
                {
                    const Token* tok1 = args[0];
                    const Token* tok3 = args[2];
                    if (tok3->str() == "(" && tok3->astOperand1() && tok3->astOperand1()->str()
                        == "sizeof" && tok3->astOperand2())
                    {

...
namespace gt
{
    ...
    bool CScope::add_child_scope(CScope* scope)
    {
        if (!scope)
        {
            return false;
    ...
    bool CScope::add_child_func(CFunction* func)
    {
        if (!func) {
            return false;
        }
    ...
    void CScope::merge(const CScope* other)
    {
        ...
        for (ScopeCI I = other->m_scopeMap.begin(), E = other->m_scopeMap.end(); I != E; ++I) {
            ...
            if (!childScope)
            {
                ...
                add_child_scope(childScope);//ignore TSC
            }
        ...
        for (FuncCI I = other->m_funcMap.begin(), E = other->m_funcMap.end(); I != E; ++I) 
        {
            ...
            if (!func)
            {
                ...
                add_child_func(func);//ignore TSC
            }
            else
            {
    ...
    public:
        ...
        void HandleAssign(const Token *tok)
        {
            ...
            if ((!pVar->isLocal() && !pVar->isArgument()))
            {
                ...
                if (vOutVarState.find(strScope) == vOutVarState.end() || !b_null)
                {
                    vOutVarState[strScope] = SOutScopeVarState(strScope, b_null);
                }
            }
            //handle argument

    ...
    const std::vector<std::string>& includePaths)
{
    ...
    bool b_ret = create_file_depend_tree(paths, includePaths, excludesPaths);
    if (!b_ret)
    {
        return false;
    }
    update_includes(includePaths);
    
    return true;
}
#ifdef _WIN32
bool CFileDependTable::build_table(CFolder* pParent, const std::string& sPath, fp_fileFilter fp)
{
    assert(pParent != NULL);
    // 如果已经有文件了，需要检查是否重复
    ...
    ////tsc
    ////if (FileLister::is_directory(strNative)) 
    {
    ...
    do 
    {
        ...
        else
        {
            ...
            bool bRet = build_table(pFolder, newPath, fp);
            if (!bRet)
                return false;
        }
    } 
...
void CFileDependTable::release_table()
{
    if (m_pRoot)
    {
        m_flag = m_begin = NULL;
...
void CFileDependTable::dump_file_depend_results()
{
    std::ofstream ofs;
    std::string sPath = get_program_directory();
    ...
    while (pFile != NULL)
    {
        ofs << Path::toNativeSeparators(pFile->GetFullPath()) << ", " << pFile->get_size() << ", expanded " << pFile->get_expand_count() << std::endl;
        std::list<CCodeFile*>::iterator iter = pFile->get_all_depends().begin();
        std::list<CCodeFile*>::iterator iterEnd = pFile->get_all_depends().end();
        iterEnd--;
        for (; iter != iterEnd; iter++)
        {
            ofs << "\t\t[" << Path::toNativeSeparators((*iter)->GetFullPath()) << ", " << (*iter)->get_size() << "]" << std::endl;
        }
        ofs << std::endl;
        pFile = pFile->get_next();
    }
...
bool CFileDependTable::create_file_depend_tree(const std::vector<std::string> &paths, const std::vector<std::string> &includePaths, const std::vector<std::string>& excludesPaths)
{
    ...
    for (std::vector<std::string>::const_iterator iter = iterBegin; iter != iterEnd; iter++)
    {
        ...
        ////if (!build_file_depend_tree(sPath, Path::acceptFile_H))
        {
            return false;
        }
    }
    ...
    {
        std::string sPath = Path::fromNativeSeparators(getAbsolutePath(*iter));
        if (!build_file_depend_tree(sPath, Path::isHeader))
        {
            return false;
        }
    }
    ...
    for (std::vector<std::string>::const_iterator iter = iterBegin; iter != iterEnd; iter++)
    {
        CFileBase* pFile = find_file(*iter);
...
void CFileDependTable::update_includes(const std::vector<std::string>& includePaths)
{
    ...
    {
        ParseIncludes(pCode);
        pCode = pCode->get_next();
    }
}

CCodeFile* CFileDependTable::get_first_file()
{
    return m_begin;
}

CFileBase* CFileDependTable::find_file(const std::string& filePath)
{
    ...
    while (end != std::string::npos)
    {
        ...
        //if (!entry.empty())
        //{
            pFile = pFolder->find_file(entry.c_str());
            if (!pFile)
                return NULL;
    ...
    return pFolder->find_file(entry);
}

bool CFileDependTable::build_file_depend_tree(std::string &sPath, fp_fileFilter fp)
{
    CFolder* pFolder = m_pRoot;
    // remove last '/' if exists
    if (*sPath.rbegin() == '/')
    ...
    ////bool b_dir = FileLister::is_directory(sNative);
    ////if (!b_dir && (!FileLister::file_exists(sPath) || !(*fp)(sPath)))
        return true;
    ...
    ////if (b_dir)
    {
        ...
        bool bRet = build_table(pFolder, sPath.c_str(), fp);
        if (!bRet)
        {
            release_table();
            return false;
...
std::string CFileDependTable::get_program_directory()
{
    std::string sPath;
#ifdef WIN32
...
bool CFileDependTable::create_log_directory(std::string* pLogPath)
{
    std::string sPath = get_program_directory();
    sPath +="log";
    ...
    ////if (!FileLister::file_exists(sPath))
    {
#ifdef WIN32
...
CFileBase* CFolder::find_file(const std::string& fileName)
{
    std::vector<CFileBase*>::iterator iter = m_subs.begin();
...
CCodeFile* CFolder::AddCodeFile(const std::string& sName, std::size_t uSize, bool& bNew)
{
    ...
    CFileBase* pFile = find_file(sName);
...
void CCodeFile::set_next(CCodeFile* next)
{
    m_next = next;
}

CCodeFile* CCodeFile::get_next()
{
    return m_next;
}

void CCodeFile::add_depend_file(CCodeFile* pFile)
{
    if (pFile)
        m_depends.push_back(pFile);
}

std::vector<CCodeFile*>& CCodeFile::get_depends()
{
    return m_depends;
}

std::size_t CCodeFile::get_size() const
{
    return m_size;
}

void CCodeFile::fill_all_depends()
{
    //m_allDepends.clear();

    std::set<CCodeFile*> included;
    expand_includes(this, m_allDepends, included);
}

std::list<CCodeFile*>& CCodeFile::get_all_depends()
{
...
void CCodeFile::expand_includes(CCodeFile* pCodeFile, std::list<CCodeFile*>& allDepends, std::set<CCodeFile*>& included)
{
    //char szOutput[1024];
    ...
    std::vector<CCodeFile*>& depends = pCodeFile->get_depends();
    ...
    for (;iter != depends.end(); iter++)
    {
        ...
        if ((*iter)->m_allDepends.empty())
        {
            ...
            expand_includes(*iter, allDepends, included);
        }
        else
        {
...
void CCodeFile::add_expand_count()
{
    ++m_nExpandCount;
}
